  ...
  const TaskModal: React.FC<TaskModalProps> = ({ isOpen, onClose, onTaskAdded }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
          ...
          if (!currentUserId && (formData.uploadedFiles && formData.uploadedFiles.length > 0) ) {
            console.warn("Attempting to upload file without an authenticated user. This might be restricted by RLS.");
          }
      } catch (authError) {
      ...
      try {
          ...
          if (uploadedFilePath && originalFileName) {
              const fileVersionData = {
                  ...
                  page_count: formData.numberOfPages ? parseInt(formData.numberOfPages) : null,
          ...
          const taskIterationData = {
              ...
              assigned_to_pm_user_id: currentUserId,
              notes: 'Task created.',
          };
          ...
          if (newFileId && newTaskIterationId) {
              ...
              if (updateFileError) {
                  console.error('Error updating file version with iteration ID:', updateFileError);
              }
          }
          ...
          const processLogData = {
              ...
              log_notes: `Project '${formData.projectName}' created. ${originalFileName ? `Initial file: ${originalFileName}` : 'No initial file.'}`,
              hours_spent_ocr: formData.estimatedHoursOCR ? parseFloat(formData.estimatedHoursOCR) : null,
              hours_spent_qc: formData.estimatedHoursQC ? parseFloat(formData.estimatedHoursQC) : null,
              hours_spent_qa: formData.estimatedHoursQA ? parseFloat(formData.estimatedHoursQA) : null,
          };
          ...
          console.log('Successfully added project and initial iteration. Project ID:', newProjectId, 'Iteration ID:', newTaskIterationId);

          setFormData({
              serialNumber: '', taskId: '', clientInstruction: '', mailInstruction: '',
              projectName: '', numberOfFiles: '', numberOfPages: '', language: '',
              processType: '', deliveryDate: '', estimatePOHours: '', listOfFiles: '',
              uploadedFiles: null, fileName: '', estimatedHoursOCR: '',
              estimatedHoursQC: '', estimatedHoursQA: ''
          });
          setCurrentPage(1);
          ...
          alert(`An unexpected error occurred: ${errorMessage}. Check console for details.`);
      } finally {
          setIsSubmitting(false);
      }
    };
        ...
        <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-screen overflow-auto pointer-events-auto">
          ...
          {currentPage === 1 ? (
            <div className="p-4">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Number of Files (Informational)</label>
                    <input
                      type="number"
                      name="numberOfFiles"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">List of Files (Informational)</label>
                  <textarea
                    name="listOfFiles"
            ...
            <form onSubmit={handleSubmit} className="p-4"> 
              <h3 className="text-lg font-semibold text-blue-600 mb-4">Form 2 - Estimated Hours</h3>
              
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Reference File Name (Optional)</label>
                  <input
                    type="text"
                    name="fileName" 
