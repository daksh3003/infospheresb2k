...
export async function GET(request: NextRequest) {
    try {
        ...
        const qaActions = taskActions.filter((action: any) => {
        ...
        const taskIds = [...new Set(qaActions.map((action: any) => action.task_id))];
        const userIds = [...new Set(qaActions.map((action: any) => action.user_id))];
