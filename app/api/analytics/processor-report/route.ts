...
export async function GET(request: NextRequest) {
    try {
        ...
        const processorActions = taskActions.filter((action: any) => {
        ...
        const taskIds = [...new Set(processorActions.map((action: any) => action.task_id))];
        const userIds = [...new Set(processorActions.map((action: any) => action.user_id))];
        ...
        processorActions.forEach((action: any) => {
        ...
        taskGroups.forEach((actions, taskId) => {
            ...
            for(let i=0; i<actions.length; i++) {
                ...
                if(['start', 'complete', 'pause', 'resume', 'upload'].includes(action.action_type)) {
                    ...
                    if(currentStage !== "Processor") continue;
